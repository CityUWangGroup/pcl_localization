  ...
  broadcaster_(this)
{
  ...
  declare_parameter("base_frame_id", "/lidar_front/points_xyzi");
...
void PCLLocalization::initialPoseReceived(geometry_msgs::msg::PoseStamped::SharedPtr msg)
{
  ...
  target_pose.header = corrent_pose_stamped_.header;
  target_pose.pose.pose = corrent_pose_stamped_.pose;
  for(int i = 0; i < 36; i++)
      target_pose.pose.covariance[i] = 0;
  pose_with_covariance_pub_->publish(target_pose);
}
...
void PCLLocalization::cloudReceived(sensor_msgs::msg::PointCloud2::ConstSharedPtr msg)
{
  ...
  RCLCPP_INFO(get_logger(), "hhb_get_info");
